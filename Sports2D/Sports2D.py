...
def parallel_process(configs):
    '''
    Run Sports2D processing in parallel for all provided configuration dictionaries
    
    Args:
        config_dicts: List of configuration dictionaries to process
        
    Returns:
        Dictionary mapping worker_id to results
    '''
    from Sports2D.parallel_process import parallel_process_fun
    ...
    for config in configs:
        if type(config) == dict:
            ...
            config_dict.append(read_config_file(config))
        ...
        for video_file, time_range, frame_rate in zip(video_files, time_ranges, frame_rates):
            ...
            parallel_process_fun(config_dict, video_file, time_range, frame_rate, result_dir)
    ...
    for i, config_dict in enumerate(config_dicts):
        thread = threading.Thread(
            ...
            args=(config_dict, i, result_queue)
        )
        threads.append(thread)
        thread.start()
        print(f"Started thread {i} for config: {config_dict.get('base').get('video_input')}")
    ...
    print(f"All {len(threads)} processing tasks completed.")
    return results
